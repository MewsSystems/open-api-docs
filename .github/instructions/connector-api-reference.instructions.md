...
- Follow the changelog rules in [CONTRIBUTING.md](../../CONTRIBUTING.md): use one GitBook `{% updates format="full" %}` block, one `{% update date="YYYY-MM-DD" %}` block per date, operation bullets with links, and the required labels such as `**Breaking:**`, `**Deprecated:**`, `**Deprecated** operation`, and `**Removed** operation`.
